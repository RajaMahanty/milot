...
export default function ProjectsPage() {
  ...
  const lastProjectElementRef = React.useCallback(
    (node: any) => {
      if (isLoading) return;
  ...
  return (
    <div
      className="flex flex-col gap-8 pb-8 h-full"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {ownedProjects.map((project, index) => (
              <ProjectCard
                ...
                onEdit={(e: React.MouseEvent) => openEditModal(e, project)}
                onShare={(e: React.MouseEvent) => openShareModal(e, project)}
...
function ProjectCard({
  ...
  return (
    ...
    >
      <div
        className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${isOwner ? "from-primary to-brand-secondary" : "from-blue-400 to-indigo-400"} opacity-40 group-hover:opacity-100 transition-opacity rounded-t-2xl`}
      />

      <div>
        <div className="flex items-start justify-between mb-2">
          <div
            className={`h-10 w-10 flex items-center justify-center rounded-xl font-bold text-xs ring-1 ring-border shadow-sm ${isOwner ? "bg-secondary text-primary" : "bg-blue-50 text-blue-600"}`}
          >
