...
export default function SprintPage() {
  ...
  const lastSprintElementRef = React.useCallback(
    (node: any) => {
      if (isLoading) return;
  ...
  if (activeProjectId === "all") {
        ...
        <div className="flex flex-col items-center justify-center h-[calc(100vh-10rem)] gap-6 animate-in fade-in zoom-in-95">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-muted-foreground text-xs max-w-xs mx-auto">
              There are no active or planned sprints in any of your workspaces.
              Switch to a specific workspace to start a sprint.
            </p>
          </div>
        </div>
      );
    ...
    // Group by project
    const sprintsByProject = visibleSprints.reduce(
      (acc, sprint) => {
      ...
      <div className="flex flex-col gap-8 pb-8 h-full animate-in fade-in slide-in-from-bottom-2">
        <div className="space-y-1">
          <h2 className="text-3xl font-bold tracking-tight text-foreground">
            All Workspace Sprints
          </h2>
          <p className="text-muted-foreground text-sm">
            Overview of active and planned sprints across your projects.
          </p>
        </div>
              ...
              <div key={projectId} className="space-y-6">
                ...
                <div className="flex flex-col gap-8">
                  {projectSprintsList.map((sprint) => {
                    let sprintTasks = allTasks.filter(
    ...
    <div className="flex flex-col gap-8 pb-8 h-full animate-in fade-in slide-in-from-bottom-2">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground text-sm">
            Manage planned and active sprints for this project.
          </p>
        </div>
        <button
      ...
      {projectSprints.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 gap-6">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-muted-foreground text-xs max-w-xs mx-auto">
              Start a new sprint to organize your team's work and track target
              goals.
            </p>
          </div>
        </div>
          ...
          {projectSprints.map((sprint, index) => {
            ...
            return (
              <div
                key={sprint.id}
                ref={
                  index === projectSprints.length - 1
                    ? lastSprintElementRef
                    : null
                }
              >
                <ProjectSprintSection
